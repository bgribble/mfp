...
def render(app_window):
    ...
    if imgui.is_window_hovered(imgui.FocusedFlags_.child_windows):
        ...
        if not isinstance(app_window.input_mgr.global_mode, ConsoleMode):
            app_window.input_mgr.global_mode = ConsoleMode(self)
    ...
    if imgui.begin_tab_bar("console_tab_bar", imgui.TabBarFlags_.none):
        if imgui.begin_tab_item("Log")[0]:
            imgui.input_text_multiline(
                'log_output_text',
                app_window.log_text,
                (app_window.window_width, app_window.console_panel_height - menu_height),
                imgui.InputTextFlags_.read_only
            )
            imgui.end_tab_item()
        if imgui.begin_tab_item("Console")[0]:
            app_window.console_manager.render(app_window.window_width, app_window.console_panel_height - menu_height)
            imgui.end_tab_item()
        imgui.end_tab_bar()

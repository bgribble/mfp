              ...
              const SerdNode * object_datatype, const SerdNode * object_lang)
{
    ...
    if (object && object->buf && (!strcmp(object->buf, "lv2:Plugin"))) {
        ...
        if (
                subject && subject->buf && pinfo->current_port_name
                && (!strcmp(subject->buf,  pinfo->current_port_name))
                && predicate && predicate->buf && (!strcmp(predicate->buf, "atom:supports"))
                && object && object->buf && (!strcmp(object->buf, "midi:MidiEvent"))
        ) {

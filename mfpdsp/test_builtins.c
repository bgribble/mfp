...
test_osc_2(void * data)
{
    ...
    for(i=0; i<((mfp_context *)data)->blocksize; i++) {
        ...
        if (fabs(100.0*sin(phase) - osc->outlet_buf[0]->data[i]) > 0.25) {
            fail = 1;
            printf("FAIL: i=%d, phase=%f, expected %f, got %f\n", i, phase,
...
test_buffer_2(void * data)
{
    ...
    /* mute logging for a bit */
    mfp_log_quiet = 1;

    mfp_dsp_schedule((mfp_context *)data);
    mfp_dsp_run((mfp_context *)data);

    /* give alloc thread time to work -- could be flaky */
    usleep(100000);
    ...
    if((info->buf_active.shm_fd == -1)
        || (info->buf_active.buf_size != blocksize*sizeof(mfp_sample))
        || (info->chan_count != 1)
        || (info->chan_size != blocksize)) {
        printf("FAIL: config %d %d %d %d %d %d\n",
                info->buf_active.shm_fd,
                info->buf_active.buf_size,
                info->chan_count,
                info->chan_size,
                blocksize,
                blocksize*sizeof(mfp_sample));
        return 0;
    }
